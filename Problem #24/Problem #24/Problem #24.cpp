...
// Note: This solution IS NOT final, as in C++ Level II
// we're going to learn how build utilities.
...
bool IsAgeValid(unsigned short age, unsigned short MinAge, unsigned short MaxAge) // passing by value. // ranges are ALWAYS passed by value.
{
    return(age >= MinAge && age <= MaxAge);
}

void PrintAgeValidity(const unsigned short &age, unsigned short MinAge, unsigned short MaxAge) // passing by constant reference.As for ranges, always by value.
{
    if (IsAgeValid(age, MinAge, MaxAge)) //   IsAgeValid() has to be a function, as procedures return void = exception thrown on this line.
    {
...
int main()
{
    ...
    PrintAgeValidity(ReadAge(InputRangeValidation), MinAge, MaxAge);  // first line to ever think of.
...
/* Tips: The nested functions line should be:
    ...
    NEW RULE: 👉 And also, THIS boolean function inside the if statement is supposed 
                 ...
                 IsAgeValid((unsigned short age, unsigned short MinAge, unsigned short MaxAge)) 
...
/* 100 pecent validated and correct:
  ...
  MinAge and MaxAge are declared in main() instead of inside the printing
